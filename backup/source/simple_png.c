...
unsigned char* decode_png(const unsigned char* png_data, int png_size, int* width, int* height) {
    ...
    *height = png_get_image_height(png_ptr, info_ptr);
    ...
    // Transform PNG to RGBA (4 bytes per pixel), then tile for 3DS textures
    if (bit_depth == 16) {
    ...
    // Ensure we have an alpha channel (RGBA) for GPU_RGBA8
    ...
    for (int y = 0; y < h; y++) {
        for (int x = 0; x < w; x++) {
            ...
            tiled[dst_idx + 0] = pixels[src_idx + 0];
            tiled[dst_idx + 1] = pixels[src_idx + 1];
            tiled[dst_idx + 2] = pixels[src_idx + 2];
            tiled[dst_idx + 3] = pixels[src_idx + 3];
        }
    }
