...
```
3ds_google_maps_src/       ← This is the actual source tree to build
  source/                  ← All .c files (all auto-compiled by SOURCES := source in Makefile)
    ...
    map_tiles.c            ← Tile coordinate math, RAM cache, rendering
    ...
    simple_png.c           ← PNG loader wrapper
    stb_image.c            ← stb_image implementation (just #define + #include)
  include/                 ← All headers
    network.h
    map_tiles.h
    gps.h
  ...
  tools/
    ...
    oracle_proxy.py        ← Old proxy script; obsolete
  Makefile
```
...
### RAM tile cache
- 64 slots, `TileCache tiles[TILE_CACHE_SIZE]` in `map_tiles.c`
- Each entry: `{int x, y, z; C2D_Image img; bool valid; u64 last_used;}`
- `get_tile(x, y, z)` → check RAM cache → check disk cache → download → parse PNG → upload to GPU texture

### Disk tile cache
- Path: `sdmc:/3ds_google_maps/tiles/{zoom}/{x}/{y}.png`
- Written by `download_url()` after successful download
- Read back by `load_tile_from_disk()` which calls stb_image to decode PNG
...
| stb_image for PNG decode | Simple, header-only, works on 3DS |
